...
def add_semantic_edges(graph: nx.DiGraph, memory: Memory, vector_store: VectorStore) -> None:
    ...
    'SIMILAR_TO' edges connecting the memories in the graph.
    """
    try:
        similar_ids = vector_store.find_similar(memory.memory_id, top_k=MAX_SIMILAR_EDGES)
...
def add_scene_edges(graph: nx.DiGraph, memory: Memory, vector_store: VectorStore) -> None:
    ...
    adds directed 'SAME_SCENE' edges connecting the memories in the graph.
    """
    try:
        similar_ids = vector_store.find_similar_scenes(memory.memory_id, top_k=MAX_SIMILAR_EDGES)
        for similar_id in similar_ids:
            if graph.has_node(similar_id):
                graph.add_edge(
                    memory.memory_id,
